...
// 챗 환경 전용 window.storage 를 일반 브라우저용 localStorage 로 대체.
// 원래 코드가 storage.get(key) -> { value } 형태와 storage.set(key, val) 을 기대하므로 그 모양을 그대로 흉내낸다.
...
async function callJSON(prompt, maxTokens, retries, useSearch = true) {
  ...
  for (let attempt = 0; attempt <= (retries ?? 2); attempt++) {
    try {
      // 우리 서버리스 함수(/api/ai)로 보낸다. 거기서 Anthropic 키를 붙여 대신 호출한다.
...
// 실시간 가격: 미국=Finnhub, 한국=야후 파이낸스. 우리 서버리스 함수(/api/quote)가 대신 호출한다.
// 실패하면 null 을 돌려주고, 그 경우 AI 가 추정한 가격을 그대로 쓴다.
async function fetchQuote(ticker, currency) {
  if (!ticker) return null;
  const market = currency === "$" ? "US" : "KR";
  try {
    const res = await fetch(`/api/quote?ticker=${encodeURIComponent(ticker)}&market=${market}`);
...
CRITICAL: for "growth" and "stable" types, "eps" MUST be the FORWARD (next 12 months / next fiscal year) consensus EPS, NEVER the trailing/TTM EPS. For "price", return the MOST RECENT available closing price (the latest trading session on or before ${today}). For the scenario fields: epsCons/perCons should be clearly LOWER than the neutral eps/per (downside case), and epsOpt/perOpt clearly HIGHER (upside case); keep them realistic, not extreme. Output numbers only for numeric fields (or null). You MUST output the JSON object even if some data is uncertain — estimate reasonably and never refuse.`;
  const d = await callJSON(prompt, 1000, 2);
  // 실시간 가격으로 덮어쓰기: 미국=Finnhub, 한국=야후. 성공하면 AI 추정가 대신 실제 시세를 쓴다.
  try {
    const live = await fetchQuote(d && d.ticker, d && d.currency);
    if (live != null) {
      d.price = live;
      d.priceSource = (d.currency === "$" ? "Finnhub 실시간" : "Yahoo Finance 실시간");
    }
  } catch (e) { /* 실시간 실패 시 AI 추정가 유지 */ }
  return d;
}
