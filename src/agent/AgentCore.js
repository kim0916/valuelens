...
// MVP 30 대표 질문 기반 패턴 업데이트
// goal: buy | fair | reco | contract | loan | region | photo | unknown

// ── 1. Session Memory ──
function createSessionMemory() {
  return {
    budget:      null,  // "7억"
    budgetNum:   null,  // 70000 (만원)
    region:      null,  // "노원구"
    purpose:     null,  // goal string
    complexName: null,  // "은마"
    area:        null,  // "34평"
    buyPrice:    null,  // 매수가 (만원)
    sellPrice:   null,  // 매도가 (만원)
    houseCount:  null,  // 보유 주택 수
    income:      null,  // 연소득 (만원)
    holdingYears:null,  // 보유기간 (년)
    jeonsePrice: null,  // 전세가 (만원)
    history:     [],
  };
}

// ── 2. Goal Inference — 우선순위 순서 배치 (20/20 통과) ──
...
function extractBasicParams(text) {
  ...
  // 예산: "7억", "5억5천", "10억5000", "15억"
  const budgetMatch = t.match(/(\d+(?:\.\d+)?)\s*억\s*(\d+천|\d+000)?/);
  if (budgetMatch) {
    const main = parseFloat(budgetMatch[1]);
    const subStr = budgetMatch[2] || "";
    const sub = subStr.includes("천") ? parseInt(subStr) * 1000
                : subStr ? parseInt(subStr) : 0;
    params.budget = `${main}억${subStr ? subStr : ""}`;
  ...
  const regions = [
    ...
    "영등포","동작","관악","은평","서대문","송도","분당","일산","판교",
    "수원","인천","의정부","안양","부천","광명","성남","하남","과천",
    "청라","검단","위례","동탄","평택","고양","용인","화성","시흥",
  ];
  ...
  const complexPatterns = [
    /([가-힣]+\s?(?:래미안|자이|힐스테이트|아이파크|롯데캐슬|푸르지오|e편한세상|더샵|sk뷰|한화포레나|파크원|리센츠|엘스|트리지움|헬리오시티|올림픽파크|주공|동부|은마|도곡|미래|현대|현대아이파크))/i,
    /([가-힣]{2,6}\s[가-힣]{1,6}(?:아파트|단지|마을|타워|팰리스))/,
  ];
  ...
  // 주택 수
  const houseMatch = t.match(/(\d)\s*주택/);
  if (houseMatch) params.houseCount = parseInt(houseMatch[1]);

  // 가격 (억 단위 → 만원)
  const prices = [...t.matchAll(/(\d+(?:\.\d+)?)\s*억/g)].map(m => parseFloat(m[1]) * 10000);
  if (prices.length >= 2) {
    params.buyPrice  = Math.min(...prices);
    params.sellPrice = Math.max(...prices);
  } else if (prices.length === 1 && !params.budgetNum) {
    params.buyPrice = prices[0];
  }
...
const REQUIRED = {
  ...
  fair:     ["complexName"],
  reco:     ["budget"],
  contract: [],   // PDF/사진 → 업로드 안내로 통일
  loan:     [],   // 세부 질문에 따라 다름 → 첫 응답에서 안내
  region:   [],   // 지역 없어도 기본 응답 가능
  photo:    [],
  unknown:  [],
};
...
const MISSING_QUESTIONS = {
  ...
  income:      "연소득을 알려주시면 대출 가능 금액을 계산해드릴 수 있어요.",
};
...
function updateMemory(memory, params) {
  ...
  if (params.jeonsePrice)  u.jeonsePrice  = params.jeonsePrice;
  return u;
}
...
function generateResponse(goal, params, memory, missingQuestion) {
  ...
  const INTROS = {
    ...
    contract: () =>
      "계약서나 등기부등본을 올려주시면\n위험 조항과 권리관계를 정리해드릴게요.\n\n📄 아래 PDF 버튼 또는 📷 사진으로 올려주세요.",

    loan: () => {
      const t = (params._rawText || "").toLowerCase();
      if (/취득세/.test(t)) return "취득세를 계산해드릴게요.\n매수 가격과 현재 보유 주택 수를 알려주세요.";
      if (/양도세/.test(t)) return "양도세를 계산해드릴게요.\n매수가, 매도가, 보유 기간을 알려주세요.";
      if (/종부세|종합부동산/.test(t)) return "종합부동산세를 계산해드릴게요.\n아파트 공시가격을 알려주세요.";
      if (/보금자리/.test(t)) return "보금자리론 자격 요건을 확인해드릴게요.\n연소득과 매수 예정 금액을 알려주세요.";
    ...
    photo: () =>
      "사진을 올려주시면 단지명·호가·면적을 자동으로 읽어 분석해드릴게요.\n📷 아래 버튼을 눌러주세요.",
  ...
  if (missingQuestion) {
    const prefix = {
      buy: "좋아요, 매수 판단을 도와드릴게요.",
      fair: "적정가를 분석해드릴게요.",
      reco: "맞춤 추천을 도와드릴게요.",
      contract: "계약 안전성을 확인해드릴게요.",
      loan: "자금 계획을 도와드릴게요.",
      region: "지역 분석을 도와드릴게요.",
      photo: "사진 분석을 시작할게요.",
      unknown: "도움이 필요하신 내용을 분석하고 있어요.",
    };
    return `${prefix[goal] || ""}\n\n${missingQuestion}`;
  }

  const fn = INTROS[goal] || INTROS.unknown;
  return fn();
}
...
function processUserInput(text, memory) {
  ...
  params._rawText = text;  // loan 세부 판단용
...
export {
  ...
  checkMissingInfo, updateMemory, generateResponse, processUserInput,
};
