...
function extractFromInput(text, intent, state) {
  ...
  // CHANGE_AREA에서 "아니 59" 같은 패턴 — 아니+숫자
  if (intent === INTENTS.CHANGE_AREA) {
    const aniNum = text.match(/아니\s+(\d{2,3})\s*$/);
    if (aniNum) { extracted.areaSqm = Number(aniNum[1]); }
  }
